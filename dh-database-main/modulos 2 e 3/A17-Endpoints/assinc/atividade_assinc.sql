...
SELECT faturas.id_cliente AS ID_CLIENTE, faturas.data_fatura AS DATA_FATURA, cancoes.nome AS MUSICA_ADQUIRIDA
FROM clientes
JOIN faturas ON clientes.id = faturas.id_cliente
...
JOIN itens_da_faturas ON faturas.id = itens_da_faturas.id
JOIN cancoes ON cancoes.id = itens_da_faturas.id_cancao
WHERE faturas.id_cliente != ' ' OR faturas.id_cliente != NULL
